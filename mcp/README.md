...
| `get_auction` | Full detail for one auction: authoritative minimum bid or live Dutch price, escrow, deadline, and whether *you* may act on it. |
...
but actually means *you are winning*.
...
early refusal explains that you are *winning* rather than failing.
...
- **Anything on chain 31337 is fair game.** Within the cap, an agent can spend the account's ETH
  ...
  on chain 31337.
- **Prompt injection can still steer *which* auctions get bid on.** NFT metadata and token URIs
  are attacker-controlled text. The server never fetches `ipfs:`/`http:` URIs for this reason,
  but inline metadata is still returned to the model. The safety model bounds *what* can happen
  and *where*, not *which* choice the model makes within those bounds.
...
(every field is a static type, so the word count *is* the field count) and decodes accordingly.
