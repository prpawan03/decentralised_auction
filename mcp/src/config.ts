...
function readMaxBidWei(): bigint {
  ...
  try {
    ...
    throw new StartupError(`AUCTION_MCP_MAX_BID_WEI must be an integer number of wei, got: ${raw}`);
  }
  ...
  if (parsed <= 0n) {
    throw new StartupError(`AUCTION_MCP_MAX_BID_WEI must be greater than zero, got: ${raw}`);
  }
  return parsed;
}
...
function readDeployment(): Deployment {
  ...
  if (houseFromEnv !== undefined) {
    if (!isAddress(houseFromEnv)) {
      throw new StartupError(`AUCTION_HOUSE_ADDRESS is not a valid address: ${houseFromEnv}`);
    }
  ...
  if (typeof auctionHouse !== "string" || !isAddress(auctionHouse)) {
    throw new StartupError(`The deployment record at ${path} has no valid "auctionHouse" address.`);
  }
  ...
  return {
    ...
    blockNumber:
      typeof blockNumber === "number" || typeof blockNumber === "string" ? BigInt(blockNumber) : 0n,
  };
}
...
export function loadConfig(): ServerConfig {
  ...
  if (!/^0x[0-9a-fA-F]{64}$/.test(rawKey)) {
    throw new StartupError("AUCTION_MCP_PRIVATE_KEY must be a 0x-prefixed 32-byte hex string.");
  }
...
export async function assertLocalChain(config: ServerConfig): Promise<void> {
  ...
  try {
    config.layout = await detectAuctionLayout(config.publicClient, config.deployment.auctionHouse);
  } catch (cause) {
