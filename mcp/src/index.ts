...
import { auctionHouseAbi, erc721Abi, AUCTION_STATUS, AUCTION_FORMAT, type AuctionStatus } from "./abi.js";
import { readAuction, readAuctionPage, type AuctionRecord } from "./auctions.js";
import { loadConfig, assertLocalChain, StartupError, localChain, type ServerConfig } from "./config.js";
...
function viewAuction(auction: AuctionTuple, id: bigint, self: Address, now: bigint) {
  ...
  return {
    ...
    format: auction.format === undefined ? null : (AUCTION_FORMAT[auction.format] ?? `unknown(${auction.format})`),
      ...
      : isDutch
        ...
        : isSeller
          ...
          : isLeader
            ...
            : null,
    canYouBuyNow: isLive && !isSeller && (isDutch || (auction.buyNowPrice !== 0n && auction.highestBid < auction.buyNowPrice)),
  ...
  server.registerTool(
    "list_auctions",
    {
      ...
      inputSchema: z.object({
        status: z
          .enum(["Live", "Settled", "Cancelled", "ReserveNotMet", "DeliveryFailed", "settleable", "any"])
          .default("Live")
          .describe(
  ...
  server.registerTool(
    ...
    async ({ auctionId }) => {
      try {
        ...
        if (auction === null) {
          return fail(
            `Auction ${id} does not exist. Call list_auctions to see the valid ids.`,
          );
        }
      ...
      try {
        ...
        const [name, symbol, owner, uri] = await Promise.all([
          ...
          config.publicClient
            ...
            .catch(() => null),
          config.publicClient
            .readContract({ address: nftAddress, abi: erc721Abi, functionName: "ownerOf", args: [id] })
            .catch(() => null),
          config.publicClient
            .readContract({ address: nftAddress, abi: erc721Abi, functionName: "tokenURI", args: [id] })
            .catch(() => null),
        ]);
