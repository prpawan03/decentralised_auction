...
async function readAllAuctions(address, total) {
  ...
  if (total > config.maxAuctions) {
    console.warn(`[collector] ${total} auctions exceeds MAX_AUCTIONS=${config.maxAuctions}; reading the first ${wanted}`);
  }
    ...
    auctions.forEach((auction, id) => {
      switch (auction.status) {
        case STATUS.LIVE:
          ...
          break;
        case STATUS.SETTLED: counts.settled++; break;
        case STATUS.CANCELLED: counts.cancelled++; break;
        case STATUS.RESERVE_NOT_MET: counts.reserveNotMet++; break;
        case STATUS.DELIVERY_FAILED: counts.deliveryFailed++; break;
...
export async function collectOnce() {
  ...
  try {
    ...
    console.warn(`[collector] chain unreachable at ${config.rpcUrl}: ${error.shortMessage ?? error.message}`);
    endPoll();
    return;
  }
