...
try {
  ...
  for (const [name, address] of Object.entries(fromEnv) as [string, Address][]) {
    ...
    if (!code || code === "0x") {
      throw new Error(
        `${name} is configured at ${address} but no bytecode exists there on ${networkName}.`,
      );
    }
  }
...
async function list(
  ...
  await send(
    nft.write.mint([seller.account.address, metadataUri(item)], {
  ...
  await send(
    house.write.createAuction([nft.address, tokenId, reserve, buyNow, duration], {
      account: seller.account,
    }),
  );
