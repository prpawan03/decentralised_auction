...
describe("Field — SC 1.3.1, 3.3.1, 3.3.2, 4.1.2", () => {
  ...
  it("links helper text with aria-describedby", () => {
    ...
    expect(document.getElementById(describedBy!.split(" ")[0]!)).toHaveTextContent("At least 2.5 ETH.");
  });
...
describe("AuctionTable — SC 1.3.1, 1.4.10", () => {
  const auctions = [
    makeAuction({ id: 1n }),
    makeAuction({ id: 2n, endTime: BigInt(NOW + 120), highestBid: 0n, highestBidder: accounts.zero }),
  ...
  it("says 'off' for a disabled buy-now, never 'free' or '0'", () => {
    renderWithProviders(
      <AuctionTable auctions={[makeAuction({ buyNowPrice: 0n })]} now={NOW} account={undefined} caption="One" />,
    );
