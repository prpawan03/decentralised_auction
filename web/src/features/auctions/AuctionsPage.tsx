    ...
    <main id="main" className="mx-auto w-full max-w-[90rem] px-4 py-6 lg:px-6">
      ...
      <div className="mt-3 flex flex-wrap items-end gap-3">
        <div className="min-w-0 flex-1 sm:max-w-sm">
          <label htmlFor="auction-search" className="col-head mb-1 block">
            Search
          </label>
        ...
        <section aria-labelledby="board-heading" className="panel min-w-0">
          ...
          {isInitialLoading ? (
            ...
            <ErrorState
              ...
              hint={
                <>
                  The app could not read <span className="tnum">{config.auctionHouseAddress}</span>{" "}
                  on {config.chainName}. Either the node at{" "}
                  <span className="tnum">{config.rpcUrl}</span> is not running, or no AuctionHouse
                  is deployed at that address. This is not a wallet problem — reads never need one.
                </>
              }
            />
          ) : isEmpty ? (
