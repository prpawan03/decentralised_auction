    ...
    return (
      ...
      >
        ...
        <p className="min-w-0 flex-1 text-[0.8125rem] text-[var(--color-ink-2)]">
          You have <Money wei={amount} size="sm" tone="live" className="align-baseline" /> waiting in
          the withdrawal vault. It stays there safely until you pull it.
        </p>
        <Button
          variant="primary"
          size="sm"
  ...
  return (
    <section aria-labelledby="withdraw-heading" className="panel">
      ...
      {error && !error.rejected && hasBalance ? (
        <p role="alert" className="border-t border-[var(--color-line)] px-4 py-3 text-[0.8125rem] text-[var(--color-danger)]">
          {error.message}
        </p>
      ) : null}
    </section>
