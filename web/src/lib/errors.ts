...
function fromCustomError(name: string, args: readonly unknown[]): DecodedError {
  ...
  switch (name) {
    ...
    case "SellerCannotBid":
      return { ...base, message: "You listed this auction. A seller cannot bid on their own item." };
    case "AlreadyHighestBidder":
      return { ...base, message: "You are already the highest bidder. Bidding again would only raise your own price." };
    case "AuctionNotLive":
      return { ...base, message: `This auction is ${statusLabel(args[1])}, so it can no longer take bids.` };
    ...
    case "AuctionHasBids":
      return {
        ...base,
        message: "This auction already has a bid, so it can no longer be cancelled. It will settle when it ends.",
      };
    case "BuyNowDisabled":
      return { ...base, message: "This auction has no buy-now price. Place a bid instead." };
    case "NothingToWithdraw":
      return { ...base, message: "You have no balance to withdraw. Refunds appear here after you are outbid." };
    ...
    case "EnforcedPause":
      return { ...base, message: "The auction house is paused. Withdrawals and settlement still work." };
...
export function decodeContractError(error: unknown): DecodedError {
  ...
  if (error instanceof BaseError) {
    ...
    if (rejection) {
      return { message: "You cancelled the request in your wallet.", rejected: true, name: "UserRejected" };
    }
    ...
    if (text.includes("nonce")) {
      return {
        message: "Your wallet's transaction count is out of step with the node. Reset the account in your wallet's advanced settings.",
        rejected: false,
        name: "NonceMismatch",
      };
    }
    if (
      ...
      return {
        message: "Cannot reach the chain node. Check that it is running and reachable at the configured RPC URL.",
        rejected: false,
        name: "NetworkError",
      };
    }
