...
type Action =
  ...
  | {
      type: "settled";
      id: string;
      status: Exclude<TxStatus, "signing" | "pending">;
      message?: string;
    }
...
function reducer(state: TrackedTx[], action: Action): TrackedTx[] {
  switch (action.type) {
    ...
    case "hash":
      return state.map((t) =>
        t.id === action.id ? { ...t, hash: action.hash, status: "pending" } : t,
      );
    case "settled":
      return state.map((t) =>
        t.id === action.id
...
export function TxTrackerProvider({ children }: { children: ReactNode }) {
  ...
  const dismiss: TxContextValue["dismiss"] = useCallback(
    (id) => dispatch({ type: "dismiss", id }),
    [],
  );
