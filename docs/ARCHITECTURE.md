...
window.__AUCTION_CONFIG__ = {
  ...
  auctionHouse: "0x5FbDB2315678afecb367f032d93F642f64180aa3",
};
```
...
2. **Reentrancy has nothing to re-enter.** The old `bid()` sent the refund
   *before* updating state, so the attacker's `receive()` ran while the contract
...
rather than strings, it can say *"Your bid must be at least 2.52 ETH"* instead
of *"Failed to place bid."*
...
*"Auction ended successfully!"* on **every** failure, including a rejected
